...
export interface Goal {
  id: string;
  /**
   * Specific instrument target (ISIN). Set this OR `instrumentType`, not both.
   * Optional so a category goal can omit it.
   */
  instrumentKey?: string;
  /**
   * Category target: ANY buy of an instrument of this type counts (e.g.
   * `tbill` = every DKJ series). Lets a recurring goal track a whole asset
   * class without pinning a single, ever-changing series.
   */
...
export function computeGoalProgress(
  ...
  return goals.map((goal) => {
    ...
    // A category goal matches any buy whose instrument has the target type;
    // otherwise we match the one specific instrument key.
    const matches = (t: Transaction): boolean => {
      if (t.type !== "buy") return false;
      if (goal.instrumentType)
        return (
          instById.get(t.instrumentKey ?? "")?.type === goal.instrumentType
        );
